...
@if($record[0]->status != 'operations')
<form action="" method="POST">
    <select class="form-control" id="options" name="notes">
...
@endif
<form action="{{ route('user.sales.updateStatus',$id) }}" method="POST" id="form" style="display: none">
    @csrf
    <label>Notes</label>
    ...
    @enderror
    <div class="row">
        <button type="submit" name="Operation" style="display: none" id="button" class="btn btn-primary">Transfer To Operation</button>


    </div>
</form>
<form action="{{ route('user.sales.storeNote',$id) }}" style="display: none" id="form2" method="POST">
    @csrf
    <textarea name="title" class="form-control"></textarea>
    <div class="row">
        <button type="submit" id="CallBack" class="btn btn-primary">Call Back</button>
        <button type="submit" id="CloseTicket" class="btn btn-danger">Close Ticket</button>
    </div>
</form>

{{-- <a href="{{ route('user.sales.updateStatus',$record[0]->id) }}" style="display: none" id="CallBack" class="btn btn-info">Call Back</a>
<a href="{{ route('user.sales.updateStatus',$record[0]->id) }}" style="display: none" id="CloseTicket" class="btn btn-danger">Close Ticket</a> --}}
@endsection
